...
const char * const CONNEX_CONF_FILENAME = "connex.conf";
const char * const CONNEX_PID_FILENAME = "connexd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "connexd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CONNEX
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CONNEX
    // Mac: ~/Library/Application Support/CONNEX
    // Unix: ~/.connex
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CONNEX";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CONNEX";
#else
    // Unix
    return pathRet / ".connex";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", CONNEX_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", CONNEX_PID_FILENAME)));
}
