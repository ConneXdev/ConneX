...
###   connexcore.org and connex.org and compares them.
...
WORKINGDIR="/tmp/connex_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://connexcore.org"
HOST2="https://connex.org"
BASEDIR="/bin/"
VERSIONPREFIX="connex-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'connex-',
...
if [ $? -ne 0 ]; then
   echo "connex.org failed to provide signature file, but connexcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "connex.org and connexcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the CONNEX Core binary release signing key installed?"
   fi
